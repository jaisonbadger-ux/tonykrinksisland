...
async function _loadProfile(uid) {
  try {
    ...
    _currentProfile = snap.exists ? snap.data() : { username: 'anon', avatarId: 'goblin' };
  } catch(e) {
...
function _renderDribzyPost(id, d, isNew) {
  ...
  const text = _esc(d.text || '').replace(/#(\w+)/g, '<span class="hashtag">#$1</span>');
  return `<div class="dribzy-post" id="dz-${id}">
    ...
    <div class="dribzy-body">
      ...
      ${d.headline ? `<div class="dribzy-source">↳ re: ${_esc(d.headline)}</div>` : ''}
